...
def thumb_start(path):

    folder_path = path
    mp4_files = find_mp4_files(folder_path)

    for mp4_file in mp4_files:
        if check_thumbnail_folder(mp4_file):
            valid_files.append(mp4_file)

    valid_files_len = len(valid_files) # 待生成的视频数量
    save_log("共计：" + str(valid_files_len) + "个文件\n")
    for valid_file in valid_files:
        file_path, file_name = os.path.split(valid_file)
        if_get_pic(file_path, file_name, valid_files_len)

    # 多线程
    # pool = multiprocessing.Pool(processes=2)
    # for file in valid_files:
    #     file_path, file_name = os.path.split(file)
    #     pool.apply_async(process_file, args=(file_path, file_name, len(valid_files)))
    #
    # pool.close()
    # pool.join()


def if_get_pic(path, f, count_f_l):
    ...
    try:
        ...
        if get_pic(path, f, count_f_l, start):  # 截取截图，如已存在截图则跳过
            ...
            save_log(
                str(path) + ':[' + str(index_count) + '/' + str(count_f_l) + ']："' + os.path.join(path,
                                                                                                  f) + '", "跳过, 预估剩余时间:' + estimated_remaining_time_formatted + '秒\n\n')
    except:
        end = time.time()  # 记录当前视频处理完成的时间
        processing_time = end - start  # 当前视频的处理时间
        total_processing_time += processing_time
        ...
        save_log(
            str(path) + ':[' + str(index_count) + '/' + str(count_f_l) + ']：' + '[----Error----],' + os.path.join(path,
                                                                                                                  f) + ', 预估剩余时间:' + estimated_remaining_time_formatted + '秒\n\n')
        #print(str(path) + ':[' + str(index_count) + '/' + str(count_f_l) + ']：' + '[Error File]', os.path.join(path, f).encode('utf-8'))
...
def get_pic(path, file, count_file_list, start):
    ...
    save_log_str += str(path) + ':[' + str(index_count) + '/' + str(count_file_list) + ']："' + os.path.join(path, file) + '",' + ' FPS：' + str(
    ...
    save_log_str += ', Done, 处理时间:' + processing_time_formatted + '预估剩余时间:' + estimated_remaining_time_formatted +'\n'
    save_log(save_log_str)
    print(save_log_str)
...
def check_thumbnail_folder(mp4_file):
    thumbnail_folder = mp4_file[:-4] + "_pics"
    if not os.path.isdir(thumbnail_folder):
        return True

    folder_size = calculate_folder_size(thumbnail_folder)
    if folder_size <= 20 * 1024:  # 20 KB
        return True
...
if __name__ == "__main__":
    ...
    with open(os.path.join(logpath, logLifecycleLog), 'a+', encoding='utf-8') as file:
        file.write(
            f"{logtime} ---- 脚本操作：运行结束----get_video_thumb_pic.py 生成略缩图----耗时 {total_time_formatted} 秒\n")
