...
logpath = '.\\'                         # 日志位置
fontpath = '.\\fonts\\Alibaba-PuHuiTi-Medium.ttf'         # 字体位置（建议用字体软件混合多种语言字体）
...
def start(path):
    ...
    for f in file_list:                         # 循环文件列表
        try:
            get_thumb(path, f)               # 截取缩略图，若返回True删除占位缩略图
            #delete_thumb(path,f)
...
def get_row(sec,col):
    ...
    elif (sec <= 3601): jg = s60        # 1小时内间隔
    elif (sec <= 5401): jg = s90        # 1.5小时内间隔
    elif (sec <= 7201): jg = s120        # 2小时内间隔
    elif (sec <= 10801): jg = s180        # 3小时内间隔
    elif (sec <= 18001): jg = s300        # 5小时内间隔
    else : jg = sot                     # 大于5小时间隔
    num = sec//jg
...
if __name__ == '__main__':
    ...
    #print('默认间隔2分钟以下：5s，10分钟：20s，30分钟：60s，1小时：120s 等等【输入数字修改，单位秒，回车跳过】')
    # s2 = int(input('2分钟内间隔：（默认：5s）') or 5)
    # s10 = int(input('10分钟内间隔：（默认：20s）') or 20)
    # s30 = int(input('30分钟内间隔：（默认：40s）') or 40)
    # s60 = int(input('1小时内间隔：（默认：1.5m）') or 90)
    # s90 = int(input('1.5小时内间隔：（默认：2m）') or 120)
    # s120 = int(input('2小时内间隔：（默认：4m）') or 240)
    # s180 = int(input('3小时内间隔：（默认：5m）') or 300)
    # s300 = int(input('5小时内间隔：（默认：8m）') or 480)
    # sot = int(input('大于5小时间隔：（默认：10m）') or 600)
